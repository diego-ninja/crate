...
Crate has two separate frontend products that must stay separate:
- `app/ui` / `crate-ui`: desktop-oriented admin web app for library management
- `app/listen` / `crate-listen`: consumer-facing listening app, designed as a PWA and intended for Capacitor packaging to app stores
...
crate-ui        (React 19 + Vite + Tailwind 4) → admin web app, desktop-oriented
crate-listen    (React 19 + Vite + Tailwind 4) → listening app, PWA/Capacitor target
...
app/ui/src/             Admin frontend
app/ui/src/pages/       Admin page components (lazy-loaded)
app/ui/src/components/  Admin shared components
app/listen/src/         Consumer listening frontend
app/listen/src/pages/   Listen pages
app/listen/src/components/ Listen shared/player components
app/shared/web/         Shared frontend core used by both apps
...
- `app/listen` stays a separate user-facing app and should be kept compatible with future Capacitor packaging
...
cd app/listen && API_URL=http://localhost:8585 npm run dev  # Listen dev server
...
- Keep `app/ui` and `app/listen` as separate apps even when extracting shared code
- Shared frontend code should live in `app/shared/web/` and be framework/product agnostic enough for both admin web and Listen/Capacitor
...
| `app/listen/src/contexts/PlayerContext.tsx` | Listen player state + queue/playback source |
| `app/listen/src/components/layout/Shell.tsx` | Listen shell for desktop/mobile layouts |
| `app/listen/src/components/layout/TopBar.tsx` | Listen global search + user menu |
