...
function getOpsForField(field: string) {
  ...
  return f.type === "text" ? TEXT_OPS : NUMBER_OPS;
}
...
export function PlaylistEditor() {
  ...
  const { data: playlist, loading, refetch } = useApi<Playlist>(
    id ? `/api/admin/system-playlists/${id}` : null,
  );
  const { data: history, refetch: refetchHistory } = useApi<GenerationLog[]>(
    id ? `/api/admin/system-playlists/${id}/generation-history` : null,
  );
  ...
  const handlePreview = useCallback(async () => {
    ...
    setPreview(null);
    try {
      // Save rules first so preview uses latest
      await api(`/api/admin/system-playlists/${id}`, "PUT", {
        smart_rules: { match, rules, limit, sort },
      });
  ...
  if (!playlist) {
    return <div className="text-center py-12 text-muted-foreground">Playlist not found</div>;
  }

  const genStatus = playlist.generation_status;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {genStatus === "running" && <Badge variant="outline" className="border-primary/30 bg-primary/10 text-primary">Generating...</Badge>}
          {genStatus === "queued" && <Badge variant="outline" className="border-amber-400/30 bg-amber-400/10 text-amber-300">Queued</Badge>}
          {genStatus === "failed" && <Badge variant="outline" className="border-red-400/30 bg-red-400/10 text-red-300">Failed</Badge>}
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-lg border border-border p-5 space-y-4">
            <h2 className="text-sm font-semibold flex items-center gap-2"><SlidersHorizontal size={14} /> Editorial</h2>
            ...
            <div>
              ...
              <textarea value={description} onChange={(e) => { setDescription(e.target.value); setDirty(true); }}
                rows={3} className="mt-1 w-full rounded-md border border-border bg-card px-3 py-2 text-sm" />
            </div>
            <div className="flex flex-wrap gap-4">
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={isActive} onChange={(e) => { setIsActive(e.target.checked); setDirty(true); }} className="accent-primary" />
                Active
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={isCurated} onChange={(e) => { setIsCurated(e.target.checked); setDirty(true); }} className="accent-primary" />
                Curated
              </label>
              {isSmart && (
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={autoRefresh} onChange={(e) => { setAutoRefresh(e.target.checked); setDirty(true); }} className="accent-primary" />
                  Auto-refresh (daily)
                </label>
              )}
              <div className="flex items-center gap-2 text-sm">
                <label className="text-muted-foreground">Rank</label>
                <input type="number" value={featuredRank ?? ""} onChange={(e) => { setFeaturedRank(e.target.value ? Number(e.target.value) : null); setDirty(true); }}
                  className="h-8 w-16 rounded-md border border-border bg-card px-2 text-sm" />
              </div>
            </div>
          </section>
          ...
          {isSmart && (
            <section className="rounded-lg border border-border p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold flex items-center gap-2"><Sparkles size={14} /> Smart Rules</h2>
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" onClick={handlePreview} disabled={previewing}>
                    {previewing ? <Loader2 size={14} className="mr-1 animate-spin" /> : <Play size={14} className="mr-1" />}
                    Preview
                  </Button>
                  <Button variant="outline" size="sm" onClick={handleGenerate} disabled={generating}>
                    {generating ? <Loader2 size={14} className="mr-1 animate-spin" /> : <RefreshCw size={14} className="mr-1" />}
                    Generate
                  </Button>
                </div>
              </div>
                ...
                {rules.map((rule, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2 rounded-md border border-border bg-card p-2">
                      ...
                      <button onClick={() => removeRule(i)} className="text-muted-foreground hover:text-red-400">
                        <X size={14} />
                      </button>
                    </div>
                  );
              ...
              {preview && (
                <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-3">
                  <div className="text-sm font-medium text-foreground">
                    {preview.total_matching} tracks match · {Math.round(preview.duration_total_sec / 60)}m total
          ...
          {/* Track list (for both smart and static) */}
          {playlist.tracks && playlist.tracks.length > 0 && (
            <section className="rounded-lg border border-border p-5">
              <h2 className="text-sm font-semibold mb-3 flex items-center gap-2">
                <ListMusic size={14} /> Tracks ({playlist.track_count})
              </h2>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="tabular-nums">{playlist.track_count}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Duration</span>
                <span className="tabular-nums">{Math.round((playlist.total_duration || 0) / 60)}m</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Followers</span>
                <span className="tabular-nums">{playlist.follower_count}</span>
              </div>
              {playlist.last_generated_at && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Last generated</span>
                  <span>{timeAgo(playlist.last_generated_at)}</span>
                </div>
              )}
              {playlist.generation_error && (
                <div className="rounded border border-red-400/20 bg-red-400/10 p-2 text-xs text-red-300">
                  {playlist.generation_error}
                </div>
              )}
            </div>
          ...
          {history && history.length > 0 && (
            <div className="rounded-lg border border-border p-4 space-y-3">
              <div className="text-xs font-medium uppercase text-muted-foreground">Generation History</div>
              <div className="space-y-2">
                {history.map((h) => (
                  <div key={h.id} className="flex items-center justify-between text-xs">
                    ...
                    {h.track_count != null && (
                      <span className="tabular-nums text-muted-foreground">{h.track_count} tracks</span>
                    )}
                  </div>
                ))}
              </div>
