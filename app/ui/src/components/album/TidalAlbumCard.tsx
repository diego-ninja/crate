...
export function TidalAlbumCard({ artist, title, year, tracks, cover, url }: TidalAlbumCardProps) {
  ...
  return (
    <div className="border border-dashed border-cyan-500/20 rounded-lg p-3 text-center group">
      <div className="w-full aspect-square rounded-md overflow-hidden mb-2 relative bg-secondary">
        ...
        <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          ...
          {status === "downloading" && (
            <div className="w-11 h-11 rounded-full bg-cyan-600/50 flex items-center justify-center">
        ...
        {status === "queued" && (
          <div className="absolute top-2 right-2 z-10">
            <Check size={14} className="text-cyan-400 drop-shadow-md" />
          </div>
        )}
      </div>
      ...
      <div className="text-xs text-muted-foreground/60 text-left flex items-center gap-1 mt-0.5">
        ...
        <span>&middot;</span>
        <span>{tracks}t</span>
        <span className="ml-auto text-cyan-500/60 text-[10px]">Tidal</span>
      </div>
    </div>
  );
}
