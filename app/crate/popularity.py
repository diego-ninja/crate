...
def recompute_track_popularity_scores(artist_names: list[str] | None = None) -> dict:
    ...
    for row in rows:
        ...
        artist_signal = (
            ...
            + 0.20 * max(0.0, min(1.0, float(row.get("artist_spotify_popularity") or 0) / 100.0))
        ...
        if has_track_level:
            raw_score = (
                0.54 * lastfm_track_signal
                + 0.12 * lastfm_rank_signal
                + 0.08 * spotify_track_signal
                + 0.16 * album_signal
                + 0.10 * artist_signal
            )
            ...
            raw_score = 0.62 * album_signal + 0.38 * artist_signal
...
def compute_popularity(progress_callback=None) -> dict:
    ...
    artist_list = sorted(touched_artists) if touched_artists else []
...
def _normalize_popularity(artist_names: list[str] | None = None):
    """Recompute consolidated track popularity and keep album ints refreshed."""
    normalize_popularity_scores()
