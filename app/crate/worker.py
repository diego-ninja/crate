...
from crate.db import update_task, save_scan_result, create_task, create_task_dedup, set_cache, get_cache, list_tasks, get_task, get_setting, get_db_ctx, emit_task_event, delete_cache
from crate.scanner import LibraryScanner
from crate.report import save_report
from crate.artwork import scan_missing_covers, fetch_cover_from_caa, save_cover
from crate.matcher import match_album, apply_match
...
def _handle_analyze_album_full(task_id: str, params: dict, config: dict) -> dict:
    ...
    from crate.audio_analysis import analyze_track, analyze_batch, PANNS_BATCH_SIZE
    from crate.db import get_library_albums, get_library_tracks, update_track_audiomuse, get_library_album
    ...
    from crate.bliss import is_available, analyze_directory, store_vectors
    bliss_count = 0
    if is_available():
...
def _handle_analyze_tracks(task_id: str, params: dict, config: dict) -> dict:
    ...
    from crate.audio_analysis import analyze_track, analyze_batch, PANNS_BATCH_SIZE
    from crate.db import get_library_albums, get_library_tracks, update_track_audiomuse
...
def _handle_delete_artist(task_id: str, params: dict, config: dict) -> dict:
    from crate.db import delete_artist as db_delete_artist, log_audit, delete_cache
    from crate.navidrome import start_scan
...
def _handle_delete_album(task_id: str, params: dict, config: dict) -> dict:
    from crate.db import delete_album as db_delete_album, log_audit, get_library_albums, upsert_artist, get_library_artist
    from crate.navidrome import start_scan
    ...
    from crate.db import get_db_ctx as _gdc
    db_path = None
    with _gdc() as cur:
        # Exact match
...
def _handle_move_artist(task_id: str, params: dict, config: dict) -> dict:
    from crate.db import get_library_artist, get_library_albums, log_audit
    from crate.navidrome import start_scan
...
def _handle_wipe_library(task_id: str, params: dict, config: dict) -> dict:
    from crate.db import wipe_library_tables, log_audit
...
def _handle_reset_enrichment(task_id: str, params: dict, config: dict) -> dict:
    from crate.db import delete_cache, get_library_artist, log_audit
...
def _handle_match_apply(task_id: str, params: dict, config: dict) -> dict:
    ...
    from crate.db import get_db_ctx, get_library_album
...
def _handle_enrich_mbids(task_id: str, params: dict, config: dict) -> dict:
    ...
    from crate.matcher import _search_musicbrainz, _get_release_detail, _score_match, _gather_local_info
    from crate.db import get_library_albums, get_library_tracks, get_db_ctx
...
def _tidal_download_inner(task_id, params, config, url, quality, download_id, lib):
    ...
    if new_release_id:
        try:
            from crate.db import mark_release_downloaded
...
def _process_new_content_inner(task_id, params, config, artist_name, album_folder):
    ...
    from crate.bliss import analyze_file as bliss_analyze, store_vectors, is_available as bliss_available
    ...
    try:
        from crate.audio_analysis import analyze_batch, PANNS_BATCH_SIZE
        analyzed = 0
        pending = []
        for album in albums:
            if album_folder and album["name"] != album_folder:
    ...
    if bliss_available():
        ...
        try:
            from crate.bliss import analyze_directory
            artist_data = get_library_artist(artist_name)
...
def _handle_scan_missing_covers(task_id: str, params: dict, config: dict) -> dict:
    ...
    from crate.artwork import scan_missing_covers, fetch_cover_from_caa, save_cover, extract_embedded_cover
    from crate.lastfm import download_artist_image
...
def _handle_popularity_chunk(task_id: str, params: dict, config: dict) -> dict:
    ...
    from crate.popularity import _lastfm_get, _parse_int, _normalize_popularity
...
def _handle_sync_playlist_navidrome(task_id: str, params: dict, config: dict) -> dict:
    ...
    from crate.navidrome import find_album, search, create_playlist as nd_create_playlist
...
def _handle_sync_shows(task_id: str, params: dict, config: dict) -> dict:
    ...
    from crate.ticketmaster import get_upcoming_shows as tm_get_shows, is_configured
    from crate.db import upsert_show, get_library_artists, delete_past_shows
