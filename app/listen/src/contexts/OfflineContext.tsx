...
interface OfflineTrackInput {
  storageId?: string | null;
  title?: string | null;
}

interface OfflineAlbumInput {
  albumId?: number | null;
  title?: string | null;
}

interface OfflinePlaylistInput {
...
function aggregateTrackState(items: OfflineItemRecord[], storageId?: string | null): OfflineItemState {
...
export function OfflineProvider({ children }: { children: ReactNode }) {
  ...
  const profileKey = useMemo(() => {
    ...
    const origin = isNative ? (getCurrentServer()?.url || window.location.origin) : window.location.origin;
  ...
  const value = useMemo<OfflineContextValue>(() => ({
    ...
    syncAll,
    clearActiveProfile,
  }), [clearActiveProfile, items, snapshot.items, summary, supported, syncAll, syncing, toggleAlbumOffline, togglePlaylistOffline, toggleTrackOffline]);
