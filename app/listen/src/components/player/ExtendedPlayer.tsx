...
export function ExtendedPlayer({ open, onClose }: ExtendedPlayerProps) {
  ...
  useEffect(() => {
    ...
    extractPalette(currentTrack.albumCover)
      .then(([c1, c2, c3]) => {
        ...
        apply();
        const firstRetry = window.setTimeout(apply, 500);
        const secondRetry = window.setTimeout(apply, 1500);
        return () => {
  ...
  }, [currentTrack?.albumCover, useAlbumPalette, vizRef]);
